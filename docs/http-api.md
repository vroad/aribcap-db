...
`aribcap-db serve` exposes archive discovery, search, raw-record, and live-stream
...
Returns records for the selected stream and month. Each entry includes its API
...
Archive records use their recording-start timestamp as the HTTP identifier.
...
appears in the record list and can be retrieved through the raw record endpoint.

The records API may include a file that is still being recorded. A raw record
...
- Unknown live streams and missing raw records return `404 Not Found`.
